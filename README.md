...
If the packaged watcher times out on your machine after opening Unity, use the repo-local single-launch watcher instead. By default it renders the standard environment instead of the separate watch executable:

`python watch_agents.py -m example_player_agent`
...
If needed, the same matchup can be run with:

`python watch_agents.py -m1 example_player_agent -m2 ceia_baseline_agent`

To force the packaged watch executable anyway, add `--watch-binary`.
